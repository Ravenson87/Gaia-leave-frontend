...
import {deleteFreeDayType, getFreeDayType} from "../../../api/freeDayType";
import CreateFreeDayType from "./developer-dashboard-role/create";
import UpdateFreeDayType from "./developer-dashboard-role/update";

const FreeDayType = () => {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'));

  const [roles, setFreeDayTypes] = useState([]);
  const [filteredFreeDayTypes, setFilteredFreeDayTypes] = useState([]);
  ...
  const [selectedFreeDayType, setSelectedFreeDayType] = useState(null);
  ...
  const fetchFreeDayTypes = async () => {
    try {
      const response = await getFreeDayType();
      if (response.status === 200) {
        setFreeDayTypes(response.data);
        setFilteredFreeDayTypes(response.data);
      } else if (response.status === 204) {
        setFreeDayTypes([]);
        setFilteredFreeDayTypes([]);
      }
    } catch (error) {
  ...
  const filterAdvancedSearch = () => {
    ...
    if (!hasAdvancedSearchValues) {
      setFilteredFreeDayTypes(roles);
      return;
    }

    if (advancedSearch.type) {
      filtered = filtered.filter(role =>
        role.type.toLowerCase().includes(advancedSearch.type.toLowerCase())
      );
    }
    ...
    setFilteredFreeDayTypes(filtered);
  };
  ...
  const handleDeleteClick = (roleId) => {
    setAlertDialog({
      open: true,
      data: {
        header: "Delete Free Day Type",
        message: "Are you sure you want to delete this free day type? This action cannot be undone.",
        type: 1,
        data: roleId
      }
    });
  ...
  const handleAlertDialogAgree = async (data) => {
    if (data.type === 1) {
      try {
        await deleteFreeDayType(data.data);
        fetchFreeDayTypes();
      } catch (error) {
      ...
      {!createModal && !updateModal && (
        <Card elevation={3} sx={{ borderRadius: 2 }}>
          ...
          <CardContent>
            {isMobile && (
              <Box>
                {filteredFreeDayTypes.length > 0 ? (
                  filteredFreeDayTypes
                    .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                    .map((role) => (
                      <FreeDayTypeCard key={role.id} role={role} />
                    ))
                ) : (
                  <Box sx={{ textAlign: 'center', py: 4 }}>
                    <Typography variant="body1" color="text.secondary">
                      No free days type found. Create one to get started.
                    </Typography>
                  </Box>
                )}
              </Box>
                ...
                <Table sx={{ minWidth: 650 }} aria-label="role table">
                  <TableHead>
                    <TableRow sx={{ backgroundColor: theme.palette.grey[50] }}>
                      ...
                      <TableCell width="15%"><Typography variant="subtitle2">Type</Typography></TableCell>
            ...
            <TablePagination
              component="div"
              count={filteredFreeDayTypes.length}
              page={page}
